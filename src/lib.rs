...
fn starlark_py(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    // Experimental sl.session namespace — class-as-namespace pattern. The
    // wrapper class is defined in Python (python/session.py); we execute
    // that source here with the underlying scoped-module primitives
    // pre-populated in globals, then attach the resulting `session` class as
    // an attribute of this module. See doc/experiments/scoped-module.md.
    let session_ns = PyDict::new(m.py());
    session_ns.set_item("ScopedModule", m.py().get_type::<ScopedModule>())?;
    session_ns.set_item("eval_scoped", m.getattr("eval_scoped")?)?;
    session_ns.set_item("eval_scoped_with", m.getattr("eval_scoped_with")?)?;
    let session_code =
        std::ffi::CString::new(include_str!("../python/session.py")).unwrap();
    m.py()
        .run(session_code.as_c_str(), Some(&session_ns), None)?;
    let session_class = session_ns
        .get_item("session")?
        .expect("session class defined in python/session.py");
    m.add("session", &session_class)?;
